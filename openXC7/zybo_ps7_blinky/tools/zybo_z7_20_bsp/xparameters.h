 ...
 * Trimmed for size: both the FSBL (sd.c) and every app here only ever
 * call f_open(..., FA_READ)/f_lseek/f_read/f_close on short 8.3 names --
 * no writing, no mkfs, no long filenames, no exFAT -- and the code these
 * pull in doesn't fit the ~63.5KB app-RAM region (ps7_ram_1) otherwise. */
